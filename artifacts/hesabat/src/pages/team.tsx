...
export function Team() {
  ...
  const roleLabel = (role: string) => t(`roles.${role as RoleId}.label`, { defaultValue: role });
  const roleDesc = (role: string) => t(`roles.${role as RoleId}.desc`, { defaultValue: "" });
  ...
  const onInviteSubmit = (data: z.infer<typeof inviteSchema>) => {
    createInvitation.mutate({ data }, {
      ...
      onError: (err: any) => {
        toast({ variant: "destructive", title: t("common.error"), description: err?.data?.error || t("team.toast.inviteError") });
      },
    });
  };
  ...
  const handleRoleChange = (id: string, role: string) => {
    updateRole.mutate({ id, data: { role: role as any } }, {
      onSuccess: () => {
        invalidateMembers();
        toast({ title: t("team.toast.roleUpdated") });
      },
      onError: (err: any) => {
        toast({ variant: "destructive", title: t("common.error"), description: err?.data?.error || t("team.toast.roleError") });
      },
    });
  };
  ...
  const handleRemoveMember = () => {
    ...
    removeMember.mutate({ id: memberToRemove.id }, {
      onSuccess: () => {
        invalidateMembers();
        toast({ title: t("team.toast.memberRemoved") });
        setMemberToRemove(null);
      },
      onError: (err: any) => {
        toast({ variant: "destructive", title: t("common.error"), description: err?.data?.error || t("team.toast.removeError") });
        setMemberToRemove(null);
      },
    });
  };
  ...
  const handleRevoke = () => {
    ...
    revokeInvitation.mutate({ id: inviteToRevoke.id }, {
      onSuccess: () => {
        invalidateInvites();
        toast({ title: t("team.toast.inviteRevoked") });
        setInviteToRevoke(null);
      },
      onError: (err: any) => {
        toast({ variant: "destructive", title: t("common.error"), description: err?.data?.error || t("team.toast.revokeError") });
        setInviteToRevoke(null);
      },
    });
  };
  ...
  const copyLink = (link: string) => {
    navigator.clipboard.writeText(link).then(
      () => toast({ title: t("team.toast.linkCopied") }),
      () => toast({ variant: "destructive", title: t("team.toast.copyFailed") }),
    );
  };
    ...
    <div className="flex flex-col">
      ...
      <div className="p-8 flex flex-col gap-8 max-w-5xl mx-auto w-full">
        ...
        <section className="flex flex-col gap-3">
          <h2 className="text-sm font-extrabold text-foreground px-1">{t("team.members")}</h2>
                ...
                {members.map((m) => (
                  <div key={m.id} className="flex items-center gap-4 px-6 py-4">
                    <div className="w-10 h-10 rounded-full bg-secondary text-secondary-foreground flex items-center justify-center font-bold uppercase shrink-0">
                      {m.name?.[0] || t("team.defaultInitial")}
                    </div>
                    <div className="flex flex-col flex-1 overflow-hidden">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold truncate">{m.name}</span>
                        {m.isSelf && (
                          <span className="text-[11px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full shrink-0">{t("common.you")}</span>
                        )}
                      </div>
        ...
        {/* Pending invitations */}
        <section className="flex flex-col gap-3">
          <h2 className="text-sm font-extrabold text-foreground px-1">{t("team.pendingInvites")}</h2>
          <div className="bg-card border rounded-2xl shadow-sm overflow-hidden">
            {invitesLoading ? (
              <div className="flex items-center justify-center p-12">
                <Spinner className="w-8 h-8 text-primary" />
              ...
              <div className="flex flex-col items-center justify-center p-10 text-muted-foreground gap-2">
                ...
                <p className="text-sm">{t("team.noPending")}</p>
              </div>
            ) : (
              <div className="flex flex-col divide-y">
                {invitations.map((inv) => (
          ...
          <div className="relative bg-card rounded-2xl shadow-2xl w-full max-w-lg border flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-foreground">{t("team.inviteNew")}</h2>
              </div>
            ...
            {createdLink ? (
              <div className="p-6 flex flex-col gap-4">
                <div className="flex flex-col gap-2 text-center">
                  ...
                  <p className="text-sm font-bold text-foreground">{t("team.linkCreatedTitle")}</p>
                  <p className="text-xs text-muted-foreground">{t("team.linkCreatedHint")}</p>
                </div>
              ...
              <form onSubmit={handleSubmit(onInviteSubmit)} className="p-6 flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-bold text-foreground">{t("team.email")}</label>
                  <input
                    type="email"
                    dir="ltr"
                    ...
                    {...register("email")}
                  />
                  {errors.email && <span className="text-xs text-destructive">{t(`team.validation.${errors.email.message}`)}</span>}
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-foreground">{t("team.role")}</label>
                  <div className="flex flex-col gap-2">
                    {ASSIGNABLE_ROLES.map((r) => (
                      <label key={r} className="flex items-start gap-3 border rounded-xl p-3 cursor-pointer hover:bg-muted/50 transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-foreground">{roleLabel(r)}</span>
                          <span className="text-xs text-muted-foreground">{roleDesc(r)}</span>
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                >
                  <UserPlus className="w-4 h-4" />
                  {createInvitation.isPending ? t("team.creating") : t("team.createLink")}
                </button>
              </form>
            )}
          </div>
      ...
      <AlertDialog open={!!memberToRemove} onOpenChange={(open) => !open && setMemberToRemove(null)}>
        <AlertDialogContent className="font-sans">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-start">{t("team.removeTitle")}</AlertDialogTitle>
            <AlertDialogDescription className="text-start">
              {t("team.removeBody", { name: memberToRemove?.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2 sm:justify-start">
            ...
            <AlertDialogAction onClick={handleRemoveMember} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">
              {removeMember.isPending ? t("team.removing") : t("team.confirmRemove")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!inviteToRevoke} onOpenChange={(open) => !open && setInviteToRevoke(null)}>
        <AlertDialogContent className="font-sans">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-start">{t("team.revokeTitle")}</AlertDialogTitle>
            <AlertDialogDescription className="text-start">
              {t("team.revokeBody", { email: inviteToRevoke?.email })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2 sm:justify-start">
            ...
            <AlertDialogAction onClick={handleRevoke} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">
              {revokeInvitation.isPending ? t("team.revoking") : t("team.confirmRevoke")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
