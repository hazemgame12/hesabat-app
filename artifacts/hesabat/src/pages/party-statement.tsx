...
import { ArrowRight, ChevronLeft, ChevronRight, Users, Warehouse } from "lucide-react";
...
type StatementResponse = {
  party: { id: string; code: string; nameAr: string; nameEn: string | null };
  data: StatementLine[];
  total: number;
  page: number;
  limit: number;
...
export default function PartyStatementPage() {
  ...
  const isCustomer = location.startsWith("/sales/");
  const ns = isCustomer ? "customers" : "suppliers";
  const backPath = isCustomer ? "/sales" : "/purchases";
  const backLabel = t(isCustomer ? "nav.sales" : "nav.purchases");
  ...
  useEffect(() => {
    ...
    fetch(`/api/${apiPath}/${id}/statement?${qs}`, { credentials: "include" })
      .then((r) => {
        if (!r.ok) throw new Error(String(r.status));
        return r.json();
      })
      .then((d: StatementResponse) => setData(d))
      .catch(() => setError(true))
      .finally(() => setLoading(false));
  }, [id, from, to, page, isCustomer]);

  const partyName = data?.party
    ? lang.startsWith("en") && data.party.nameEn
      ? data.party.nameEn
      : data.party.nameAr
    : "";
  ...
  const Icon = isCustomer ? Users : Warehouse;
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-foreground">{partyName || "..."}</h1>
              {data?.party && (
            ...
            <p className="text-sm text-muted-foreground">{t(`${ns}.statement`)}</p>
          </div>
        </div>
