...
export function InvoicePaymentAllocations({
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/invoices/${invoiceId}/available-payments`);
      if (res.ok) {
  ...
  const handleAllocate = async () => {
    ...
    if (!selectedPaymentId || isNaN(amount) || amount <= MONEY_EPS) {
      toast({
        title: t("invoices.allocations.invalidAmount"),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast({
          title: d.error ?? t("invoices.allocations.error"),
          variant: "destructive",
        });
        return;
      }
              ...
              {existingAllocations.map((a) => (
                ...
                >
                  <div className="flex items-center gap-3 text-muted-foreground text-xs">
                    <span>
                      {t("invoices.paymentNo")} {a.paymentNo ?? "—"}
                    </span>
                ...
                <>
                  ...
                  <div className="flex gap-2 flex-wrap items-end">
                    <div className="flex-1 min-w-[180px]">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const p = availablePayments.find(
                            (x) => x.id === e.target.value,
                          );
                          if (p)
                            setAllocAmount(String(r2(p.unallocatedAmount)));
                          else setAllocAmount("");
                        }}
                    ...
                    <button
                      ...
                      disabled={
                        saving || !selectedPaymentId || !allocAmount
                      }
