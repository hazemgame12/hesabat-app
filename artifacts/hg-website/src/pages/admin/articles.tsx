...
import {
  ...
  clearAdminToken, getAdminToken, type ArticleRecord,
} from "@/lib/api";
...
export default function AdminArticles() {
  ...
  const handleDelete = async (id: number, titleAr: string) => {
    if (!token || !confirm(`هل تريد حذف المقال: "${titleAr}"؟`)) return;
  ...
  return (
    <AdminLayout title="إدارة المقالات">
      <div className="flex items-center justify-between mb-6">
        <p className="text-gray-500 text-sm">{articles.length} مقال في قاعدة البيانات</p>
        <Link href={`${base}/admin/articles/new`}>
          <Button className="gap-2 h-10"><Plus className="w-4 h-4" />مقال جديد</Button>
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400">
          ...
          <Link href={`${base}/admin/articles/new`}><Button>أضف أول مقال</Button></Link>
        </div>
      ) : (
              ...
              {articles.map((article) => (
                <tr key={article.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-4 hidden md:table-cell">
                    <span className="bg-primary/10 text-primary text-xs font-semibold px-2.5 py-1 rounded-full">{article.categoryAr}</span>
                  </td>
                  <td className="py-4 px-4 hidden lg:table-cell text-gray-500">{formatDate(article.date)}</td>
                  <td className="py-4 px-4 text-center">
                    <button onClick={() => handleToggle(article)}
                      className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full transition-colors ${article.published ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button onClick={() => handleDelete(article.id, article.titleAr)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
