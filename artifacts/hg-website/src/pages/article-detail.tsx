...
import { Clock, Tag, ArrowLeft, ArrowRight, ChevronLeft, ChevronRight } from "lucide-react";
...
function renderMarkdown(text: string) {
  ...
  for (const line of lines) {
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <li key={key++} className="text-muted-foreground leading-relaxed my-1 ms-4 list-decimal">
          {content}
        </li>
      );
    } else if (line.trim() === "") {
      ...
      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-3xl mx-auto">
            ...
            <Link
              href={`${base}/articles`}
              className="inline-flex items-center gap-2 text-primary font-semibold mb-10 hover:gap-3 transition-all"
            >
            ...
            <div className="mt-16 bg-[#001d56] rounded-3xl p-8 text-white text-center">
              <h3 className="text-2xl font-bold mb-3">
                {lang === "ar" ? "هل تحتاج مساعدة متخصصة؟" : "Need Expert Help?"}
              </h3>
              <p className="text-gray-300 mb-6">
                {lang === "ar"
                  ? "فريقنا من الخبراء جاهز للإجابة على استفساراتك وتقديم الحل الأمثل لشركتك"
                  : "Our team of experts is ready to answer your questions and provide the best solution for your business"}
              </p>
              <Link
                href={`${base}/`}
                onClick={() => setTimeout(() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" }), 100)}
                className="inline-block bg-primary hover:bg-primary/90 text-white px-8 py-3 rounded-xl font-bold transition-colors"
              >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {related.map((rel) => (
              <Link
                ...
                className="group block rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all bg-white dark:bg-card border border-gray-100 dark:border-gray-800"
              >
                <div className="h-40 overflow-hidden">
                  <img src={rel.image} alt={rel.title[lang]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <div className="p-5">
                  <span className="text-xs text-primary font-semibold">{rel.category[lang]}</span>
