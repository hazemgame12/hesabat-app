...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const deletableIds = candidates.map((e) => e.id);
      const attachmentKeys = await db
        .select({ objectKey: journalEntryAttachmentsTable.objectKey })
        .from(journalEntryAttachmentsTable)
        .where(
          and(
            inArray(journalEntryAttachmentsTable.entryId, deletableIds),
            eq(journalEntryAttachmentsTable.companyId, companyId),
          ),
        );
      await db
        ...
        .where(
          and(
            inArray(journalEntriesTable.id, deletableIds),
            eq(journalEntriesTable.companyId, companyId),
          ),
        );
      for (const { objectKey } of attachmentKeys) {
        fs.promises.unlink(path.join(uploadsDir, objectKey)).catch(() => {});
      }
      await safeAudit(
        db,
        {
          ...
          entity: "journal_entry",
          entityId: companyId,
          entityLabel: `حذف جماعي ${deletableIds.length} قيد`,
          oldValue: { count: deletableIds.length },
        },
        req.log,
      );
      res.json({ deleted: deletableIds.length, skipped: ids.length - deletableIds.length });
    } catch (err) {
      req.log.error({ err }, "Failed to bulk delete journal entries");
      res.status(500).json({ error: "حدث خطأ في الخادم" });
    }
  },
);

// ---- Bulk reverse (posted only) ----
router.post(
  "/journal/bulk-reverse",
  requireAuth,
  requireCapability("journal:create"),
  async (req, res) => {
    const companyId = req.auth!.companyId;
    ...
    try {
      ...
      if (candidates.length === 0) {
        res.json({ reversed: 0, skipped: ids.length });
        return;
      }
      const existingReversals = await db
        .select({ reversedEntryId: journalEntriesTable.reversedEntryId })
        .from(journalEntriesTable)
        .where(
          and(
            eq(journalEntriesTable.companyId, companyId),
            inArray(
              journalEntriesTable.reversedEntryId,
              candidates.map((e) => e.id),
            ),
            eq(journalEntriesTable.entryType, "reversal"),
          ),
        );
      const alreadyReversedSet = new Set(
        existingReversals.map((e) => e.reversedEntryId!),
      );
      const toReverse = candidates.filter((e) => !alreadyReversedSet.has(e.id));
      ...
      for (const original of toReverse) {
        try {
          const sourceLines = await db
            ...
            .where(
              and(
                eq(journalEntryLinesTable.entryId, original.id),
                eq(journalEntryLinesTable.companyId, companyId),
              ),
            )
            .orderBy(asc(journalEntryLinesTable.lineNo));
          await db.transaction(async (tx) => {
            ...
            const [existingReversal] = await tx
              ...
              .where(
                and(
                  eq(journalEntriesTable.companyId, companyId),
                  eq(journalEntriesTable.reversedEntryId, original.id),
                  eq(journalEntriesTable.entryType, "reversal"),
                ),
              )
              .limit(1);
            ...
            const [entry] = await tx
              ...
              .values({
                ...
                date: today,
                reference: original.reference,
                notes: `قيد عكسي للقيد ${formatEntryNumber(original.entryNo, original.date)}`,
                status: "draft",
                entryType: "reversal",
                reversedEntryId: original.id,
                createdBy: req.auth!.userId,
          ...
          req.log.warn(
            { err: innerErr, entryId: original.id },
            "Skipped one entry in bulk reverse",
          );
        }
      }
      await safeAudit(
        db,
        {
          ...
          entity: "journal_entry",
          entityId: companyId,
          entityLabel: `عكس جماعي ${reversed} قيد`,
          oldValue: { count: reversed },
        },
        req.log,
      );
